...
// PORT: same reasoning as sc_server_set_server_path -- the host already knows
// where its bundled adb.exe is, and setting the ADB env var would leak into
// every child process it spawns.
static char *adb_executable_override;

void
sc_adb_set_executable(const char *path) {
    free(adb_executable_override);
...
adb_resolve_executable(void) {
    if (adb_executable_override) {
        ...
        LOGD("Using adb set by host: %s", adb_executable);
        return true;
    }
