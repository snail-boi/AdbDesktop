...
// PORT: set natively by the host instead of via the SCRCPY_SERVER_PATH env
// var. An env var would be inherited by every process the host spawns later
// (it runs its own adb commands), which is both leaky and racy.
...
get_server_path(void) {
    if (server_path_override) {
        LOGD("Using server path set by host: %s", server_path_override);
        char *copy = strdup(server_path_override);
        if (!copy) {
            LOG_OOM();
        }
        return copy;
