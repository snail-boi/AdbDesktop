...
#include "receiver.h"
// PORT: upstream includes "util/acksync.h" here, and gets sc_uhid_devices via
// receiver.h -> uhid/uhid_output.h. Neither subsystem is part of this build
// (see receiver.h), so both types are forward-declared at file scope. They are
// used only as opaque pointers in sc_controller_configure(), and every caller
// passes NULL.
struct sc_acksync;
