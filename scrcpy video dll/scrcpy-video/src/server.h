...
/*
 * PORT: override the scrcpy-server location natively.
 *
 * Upstream only reads SCRCPY_SERVER_PATH from the environment, but this is a
 * DLL inside a host that spawns its own adb processes -- an env var would be
 * inherited by all of them. Pass NULL to fall back to upstream behaviour.
 */
void
