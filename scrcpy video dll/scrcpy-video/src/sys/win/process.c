...
sc_process_execute_p(const char *const argv[], HANDLE *handle, unsigned flags,
    ...
    if (!inherit_stdout && !inherit_stderr) {
        ...
        // PORT: this library runs inside a windowed host process that has no
        // console, so console children (adb) would otherwise each flash up a
        ...
        // (Mutually exclusive with DETACHED_PROCESS, hence the else.)
