...
// PORT: this file declares vasprintf() and reallocarray(), which need va_list
// and size_t. Upstream never included these headers -- it got both types by
// accident, transitively via <SDL3/SDL_version.h>. Dropping SDL exposed the
// latent dependency, so they are now included on purpose.
#include <stdarg.h>
#include <stddef.h>
...
// PORT: upstream also includes <SDL3/SDL_version.h> here, but nothing in this
// file references an SDL macro -- it was vestigial. Dropped along with the rest
// of the SDL dependency.
//
// libavformat is included for its version macros only; this build links
// avcodec, avutil and swscale, not avformat.
